...
def save_result():
    ...
    with open(os.path.join(log_dir, session_id + '_system_for_record.txt'), "a+") as f:
        f.write("[" + str(time.time() - local_time_start) + "] " + system_response + "\n")
    return "OK"

@app.route('/underlineClicked', methods=['POST'])
def underlineClicked():
    global local_time_start
    with open(os.path.join(log_dir, request.form['sessionkey'] + "_clicked.txt"), "a+") as f:
        f.write("[" + str(time.time() - local_time_start) + "]" + ' underline' + "\n")
    return "OK"
...
def generate():
    ...
    session_id = request.form['sessionkey']
    with open(os.path.join(log_dir, request.form['sessionkey'] + "_clicked.txt"), "a+") as f:
        f.write("[" + str(time.time() - local_time_start) + "]" + ' submit' + "\n")
    with open(os.path.join(log_dir, session_id + "_user_for_record.txt"), "a+") as f:
        f.write("[" + str(time.time() - local_time_start) + "] " + user_input + "\n")
...
def highlight():
    ...
    with open(os.path.join(log_dir, session_id + "_system_for_record.txt"), "a+") as f:
        f.write("[" + str(time.time() - local_time_start) + "] " + answer_response + "\n")
    while True:
        try:
            ...
            with open(os.path.join(log_dir, session_id + "_unclear_system_for_record.txt"), "a+") as f:
                f.write("[" + str(time.time() - local_time_start) + "] " + annotation_response + "\n")
    ...
    with open(os.path.join(log_dir, session_id + "_text_for_record.txt"), "a+") as f:
        f.write("[" + str(time.time() - local_time_start) + "] " + user_input + answer_response + "\n")
    print("======================= HIGHLIGHT ==================== \n")
    print(user_input)
    print("\n")
    ...
    while True:
        try:
            ...
            for item in msg_splitted:
                ...
                print(input_ori, input_mod)
                repeated = False
                for judging in history_splitter:
                    if input_ori in judging:
                        repeated = True
                        break
                if not repeated:
                    print("NOT REPEATED")
                    history_splitter.append(input_ori)
                    regex = re.compile(input_ori)
...
def refine():
    try:
        ...
        global advice_text_to_refine
        global answer_reserve
        now_time = time.time() - local_time_start
        with open(os.path.join(log_dir, request.form['sessionkey'] + "_clicked.txt"), "a+") as f:
            f.write("[" + str(now_time) + "]" + ' refine' + "\n")
        with open(os.path.join(log_dir, session_id + "_highlight_text_refine_for_record.txt"), "a+") as f:
            f.write("[" + str(now_time) + "]" + str(highlight_text_to_refine) + "\n")
        with open(os.path.join(log_dir, session_id + "_advice_text_refine_for_record.txt"), "a+") as f:
            f.write("[" + str(now_time) + "]" + str(advice_text_to_refine) + "\n")
        with open(os.path.join(log_dir, session_id + "_answer_reserve.txt"), "a+") as f:
            f.write("[" + str(now_time) + "]" + str(answer_reserve) + "\n")
        with open(os.path.join(log_dir, session_id + "_user_for_record.txt"), "a+") as f:
            f.write("[" + str(now_time) + "]" + str(now_input) + "\n")
        with open(os.path.join(log_dir, session_id + "_system_for_record.txt"), "a+") as f:
            f.write("[" + str(now_time) + "]" + str(now_answer) + "\n")
        messages = [
